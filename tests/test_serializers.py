...
from api.serializers.questionnaire_reponse import QuestionnaireResponseSerializer
...
@pytest.mark.parametrize(
    "resource",
    [
        "UKCore-QuestionnaireResponse-BabyPatientSurvey-Example",
    ],
)
def test_questionnaire_response(resource, baby_patient, doctor_paul_rastall):
    with open(f"{TEST_DIR}/data/{resource}.json") as f:
        payload = json.load(f)
